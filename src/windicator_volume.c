...
void windicator_volume_icon_set_by_level(Evas_Object *icon, void *data)
{
	...
	if (ad->volume_index)
	    elm_object_signal_emit(icon, "volume.icon.media", "img.volume.icon");
	else
		elm_object_signal_emit(icon, "volume.icon.mute", "img.volume.icon");
}
...
static int _current_device_volume_level_get(void* data)
{
	...
	int volume_index = 0, real_volume_index = 0;
	_D("Music app is NOT ongoing, so get SOUND_TYPE_RINGTONE's level");
	...
	_D("Setting - vconf vol: %d, real vol: %d", volume_index, real_volume_index);
